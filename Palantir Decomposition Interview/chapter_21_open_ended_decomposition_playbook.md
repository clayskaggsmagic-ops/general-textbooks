# Chapter 21: The Decomposition Playbook — Executing Palantir's Signature Interview
...
## 21.1 What Makes Decomposition Different from a Consulting Case
...
## 21.2 The Anatomy of 45 Minutes
...
## 21.3 The Five Moves — Decomposition-Specific Techniques
...
## 21.4 The Five Types of Interviewer Pushback
...
## 21.5 The Cognitive Traps — What Kills Candidates
...
## 21.6 The Whiteboard: Visual Decomposition Techniques
...
## 21.7 The Opening Script — Your First 3 Minutes
...
## 21.8 Worked Decomposition — Full 45-Minute Simulation
...
## 21.9 Rapid-Fire Practice Prompts
...
## 21.10 The Questions You Must Ask — Universal Playbook

No matter the prompt, your first 3–5 minutes should be spent asking questions, not building solutions. Interviewers rank candidates higher when they ask before they build. Here is the universal question set — categorized so you can pick 4–5 without thinking.

### Tier 1: Always Ask These (Pick 2–3)

| # | Question | Why It Matters |
|---|----------|----------------|
| 1 | **"Who is the primary user, and what decision do they make with this?"** | Defines the entire UX. A field worker needs a mobile alert; a VP needs a weekly scorecard. |
| 2 | **"What data exists today, and at what granularity?"** | Prevents you from designing on top of data that doesn't exist. Also reveals data quality reality. |
| 3 | **"How is this problem handled right now — manually, in spreadsheets, or not at all?"** | Uncovers the baseline. If the current process is "email chains," the bar is low. If they already have a BI tool, you need to differentiate. |

### Tier 2: Scope & Constraints (Pick 1–2)

| # | Question | Why It Matters |
|---|----------|----------------|
| 4 | **"What does success look like in 6 months — a specific metric, or a capability?"** | Forces the interviewer to clarify whether this is a KPI-reduction problem or a capability-building problem. |
| 5 | **"Are there regulatory, compliance, or political constraints?"** | In healthcare, defense, fintech — the constraints shape the solution more than the data does. |
| 6 | **"What's the geography and scale?"** | A 50-store chain vs. 5,000 stores means completely different deployment strategies. |

### Tier 3: Deployment Reality (Ask If Time Allows)

| # | Question | Why It Matters |
|---|----------|----------------|
| 7 | **"Has this been attempted before? What happened?"** | Reveals organizational scar tissue. If a previous system failed, you need to address adoption. |
| 8 | **"Who would resist this, and why?"** | Shows you understand change management — a key DS skill. |

### Anti-Pattern Questions — What NOT to Ask

| Don't Ask | Why It's Bad |
|-----------|-------------|
| "What algorithm should I use?" | You decide the approach. Don't outsource that to the interviewer. |
| "Can you tell me more about the data?" (vaguely) | Too passive. Propose what data you'd *expect* and ask them to confirm. |
| "How much time do I have?" | You know the format. Don't waste a question on logistics. |

---

## 21.11 The Biggest Design Decisions — Framework

Every decomposition forces 3–5 major design decisions. Interviewers are explicitly testing whether you identify these decisions, articulate the trade-offs, and pick a side with reasoning. Here are the recurring archetypes.

### Decision 1: Prediction vs. Automation

**The question:** Should the system *predict* what will happen and surface it to a human, or should it *automate* the response?

| Factor | Lean Prediction | Lean Automation |
|--------|----------------|-----------------|
| **Stakes** | High (child welfare, military) | Low–medium (inventory reorder, scheduling) |
| **Legal liability** | Human must be accountable | No legal exposure |
| **User trust** | Low — system is new | High — users trust the logic |
| **Data quality** | Noisy, incomplete | Clean, well-understood |

**How to say it:** "I'd start with prediction and decision support — surface the risk to the user and let them act. Once the model proves itself over 6 months and users trust it, we can automate low-stakes actions like reorder triggers while keeping high-stakes actions human-in-the-loop."

---

### Decision 2: Real-Time vs. Batch

**The question:** Does the system need to react in seconds, or is a daily/weekly refresh sufficient?

| Factor | Lean Real-Time | Lean Batch |
|--------|---------------|------------|
| **Decision cadence** | Seconds–minutes (dispatch, alerts) | Hours–days (planning, reporting) |
| **Data freshness** | Sensor/IoT, streaming | Warehouse, ERP, monthly reports |
| **Infrastructure cost** | High (streaming pipelines) | Low (scheduled jobs) |
| **Error tolerance** | Low — wrong alert = missed event | Higher — weekly report can wait |

**How to say it:** "The dispatch use-case needs real-time — a field technician can't wait for a daily batch to learn their next job. But the executive dashboard? That can be overnight batch. I'd build both: a real-time action layer and a batch analytics layer."

---

### Decision 3: Build vs. Integrate

**The question:** Should we build a new data source, or integrate what already exists?

| Factor | Build New | Integrate Existing |
|--------|----------|-------------------|
| **Data gap** | No existing source covers this | Data exists but is siloed |
| **Speed** | Slow (weeks–months to instrument) | Fast (days if API/export exists) |
| **Accuracy** | High (designed for purpose) | Variable (wasn't built for this) |

**How to say it:** "The ideal answer includes IoT sensor data, but they probably don't have that yet. I'd start by integrating the maintenance logs and inspection records they already have — that gets us 70% of the signal. Then in Phase 2, we'd pilot sensors on the 50 highest-risk assets."

---

### Decision 4: Centralized Platform vs. Federated Views

**The question:** One system for everyone, or tailored views per user group?

**When it matters:** Government agencies, multi-division companies, international operations.

**The right answer (almost always):** "One Ontology, multiple Workshop apps. The data model is shared — that's the source of truth. But the executive sees a scorecard, the field team sees a task list, and the analyst sees the full pipeline. Same data, different surfaces."

---

### Decision 5: Precision vs. Recall

**The question:** Is it worse to miss a real case (low recall) or to flood users with false alarms (low precision)?

| Domain | Optimize For | Reasoning |
|--------|-------------|-----------|
| Wildfire prediction, child welfare, fraud | **Recall** — catch everything | A false negative is catastrophic |
| Alert fatigue reduction, spam filtering | **Precision** — reduce noise | Too many false positives = users ignore the system |
| Medical screening | **Recall** (screening) → **Precision** (confirmation) | Two-stage: cast wide net, then confirm |

**How to say it:** "The asymmetry here is extreme. Missing a real wildfire-risk line and having someone die is infinitely worse than inspecting a line that turns out to be fine. So I'd tune for high recall — catch every risky segment — and accept the false positive cost, which is just an unnecessary inspection."

---

### The Decision Matrix — Use This in Any Interview

When you hit a design fork, use this 30-second framework:

```
1. Name the decision: "We have a choice between X and Y."
2. State the trade-off: "X gives us __, but costs __. Y gives us __, but risks __."
3. Recommend: "Given [the stakes / the user / the timeline], I'd lean toward X."
4. Hedge: "But I'd validate this with the client in the first week."
```

This takes 30 seconds, demonstrates senior thinking, and gives the interviewer a natural hook to push back on — which is exactly what you want.

---

## 21.11 Move 6: Define Success Metrics (Don't Wait for Them to Ask)

Most candidates design a beautiful system and then get caught flat-footed when the interviewer asks: **"How would you know if this is working?"** Don't wait for that question. Build metrics into your decomposition proactively — it shows senior thinking and deployment awareness.

### The Three-Level Metrics Stack

For every system you design, define metrics at three levels:

| Level | What It Measures | Example (Grocery Waste System) |
|-------|-----------------|-------------------------------|
| **Data Health** | Is the system itself working? | "95% of stores reporting, data refreshed every 15 min" |
| **Usage** | Are people actually using it? | "85% of store managers log in daily, 60% of alerts acted on" |
| **Outcome** | Did the business improve? | "22% less spoilage vs. last quarter, $88M recovered" |

### How to Deliver It in the Interview

After your Ontology close (Move 5), add 60 seconds on metrics:

> *"To measure success, I'd define three layers. The North Star Metric is [outcome metric] — that's what the client ultimately cares about. Supporting that, I'd track [usage metric] to make sure humans are actually engaging with the tool, and [data health metric] to make sure the underlying data is trustworthy. If usage is high but outcomes aren't improving, that tells us the recommendations need tuning. If usage is low, the problem is adoption, not the model."*

### The "Leading vs. Lagging" Move

If you want to sound especially sharp, name one **leading** indicator (predicts future success) and one **lagging** indicator (confirms past success):

> *"A leading indicator would be how quickly managers respond to waste alerts — if response time drops from 4 hours to 30 minutes, we'll see spoilage decrease. The lagging indicator is the quarterly waste number — that's the proof the system worked."*

### Why This Move Matters

- **It shows deployment mindset.** You're not just designing; you're thinking about accountability.
- **It gives the interviewer a natural follow-up.** They'll ask "what if the metrics don't improve?" — which lets you demonstrate iterative thinking.
- **It differentiates you.** Most candidates never mention measurement. You will.

> **For more depth on metrics, Fermi estimation, and analytical reasoning:** See Appendix J — Analytical Problem Solving.

---

## 21.12 Chapter Summary — Quick Reference
...
6. **Define Success Metrics** — North Star metric, leading vs. lagging indicators

### The Phased Deployment Template

Every decomposition answer should end with a rollout plan. Use this 3-phase pattern:

```
PHASE 1: PROVE IT (Weeks 1-4)
├── Integrate core data sources
├── Build descriptive dashboard (visibility only)
├── Deploy to ONE team/site/region as pilot
└── Success gate: Users log in daily, data is accurate

PHASE 2: POWER IT (Months 2-3)
├── Add recommendations / predictive features
├── Enable Actions (users can act, not just see)
├── Expand to 3-5 teams/sites based on pilot results
└── Success gate: Measurable outcome improvement at pilot sites

PHASE 3: SCALE IT (Months 4-6)
├── Org-wide rollout
├── Add AIP layer (natural language, automation)
├── Integrate with downstream systems
└── Success gate: North Star metric improves at org level
```

**Why this matters:** Saying "I'd deploy it in phases" is vague. Naming the phases, the scope of each, and the success gates that unlock the next phase shows deployment maturity. It also gives the interviewer a natural follow-up: "What if Phase 1 fails?" — which lets you show resilience and iteration.
...
*Next Chapter: Chapter 22 — Decomposition Practice Lab: Eight Complete Scenarios with Analysis →*
