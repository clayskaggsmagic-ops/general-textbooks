...
**Key difference from the current architecture**: The document IS the state. There's no `final_state` TEXT column containing a serialized copy — the document *is* the canonical representation. LangGraph's state maps 1:1 to this document, eliminating the "shadow state" anti-pattern from Chapter 27.
